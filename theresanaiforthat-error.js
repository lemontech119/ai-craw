...
const productDetailCrawlerBoolean = async (browser, crawlerData, i) => {
  try {
    ...
    await util.delay(util.getRandomDelay(1000, 5000));
...
const crawler = async () => {
  ...
  const errorJsonString = fs.readFileSync("에러파일.json", "utf-8");
  const errorJsonList = JSON.parse(errorJsonString);
  errorJsonList.forEach((element) => {
    data.push({
      ranking: 0,
      url: element.url,
    });
  ...
  console.log("errorJsonList count", data.length);

  // console.log("data count", data.length);

  await productDetailCrawler(browser, errorJsonList);
